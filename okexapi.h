#ifndef __OKEXAPI_H__
#define __OKEXAPI_H__
...
public:
	...
	string GetTicker(string &symbol);																								//Get /api/v1/ticker						获取Okex行情
	string GetDepth(string &symbol,string &size,string &merge);																		//Get /api/v1/depth							获取Okex市场深度
	string GetTrades(string &symbol,string &since);							//Get /api/v1/trades						获取Okex最近600交易信息
	...
	string DoTrade_History(string &symbol,string &since);																			//POST /api/v1/trade_history				获取Okex历史交易信息
...
class OkexApiCom:public OkexApi	//国际站
{
public:
	OkexApiCom(string api_key,string secret_key){
	...
	string DoFuture_Ticker(string &symbol,string &contract_type);																	//GET /api/v1/future_ticker				获取Okex期货行情
	string DoFuture_Depth(string &symbol,string &contract_type,string &size,string &merge);											//GET /api/v1/future_depth				获取Okex期货深度信息
	string DoFuture_Trades(string &symbol,string &contract_type);																	//GET /api/v1/future_trades				获取Okex期货交易记录信息
	string DoFuture_Index(string &symbol);																							//GET /api/v1/future_index				获取Okex期货指数信息
	...
	//期货交易 API
	//用于Okex快速进行期货交易
	string DoFuture_Userinfo();																															//POST /api/v1/future_userinfo			获取Okex期货账户信息 （全仓）
	string DoFuture_Position(string &symbol,string &contract_type);																						//POST /api/v1/future_position			获取用户持仓获取Okex期货账户信息 （全仓）
	...
	string DoFuture_Trades_history(string &symbol,string &date,string &since);																			//POST /api/v1/future_trades_history	获取Okex期货交易历史
