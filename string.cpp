...
string str::UnicodeToAnsi( const wstring &wstrSource)
{
    ...
    if (wstrSource.length() == 0)
    return "";

    std::string str;
    str.assign (wstrSource.begin(), wstrSource.end());
    return str;
};
...
//	free(pszDst);
//	return strTemp;
    wstring tmp = wstring(pwzsSource);
    strTemp.assign(tmp.begin(), tmp.end());
    return strTemp;
};
